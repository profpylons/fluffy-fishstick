...
function handleToolsList() {
    ...
    return new Response(JSON.stringify({ tools }), { headers: CORS_HEADERS });
}
/**
 * Handle tool execution endpoint - executes a specific tool
 */
async function handleToolExecution(request, env) {
    try {
        const body = await request.json();
        const { name, arguments: args } = body;
        // Validate tool name
        if (name !== fetchGameDataTool.name) {
            return new Response(JSON.stringify({ error: `Unknown tool: ${name}` }), { status: 404, headers: CORS_HEADERS });
        }
